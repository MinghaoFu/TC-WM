...
from . import custom_resolvers  # registers Hydra resolvers (replace_slash)
...
def slice_trajdict_with_t(data_dict, start_idx=0, end_idx=None, step=1):
    if end_idx is None:
        end_idx = max(arr.shape[1] for arr in data_dict.values() if arr is not None)
    return {key: (arr[:, start_idx:end_idx:step, ...] if arr is not None else None) for key, arr in data_dict.items()}
...
def concat_trajdict(dcts):
    ...
    for k in dcts[0].keys():
        if dcts[0][k] is None:
            full_dct[k] = None
        elif isinstance(dcts[0][k], np.ndarray):
